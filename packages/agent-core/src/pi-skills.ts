/**
 * Bridge QuantumAI AgentSkillRuntime (employee policy + progressive disclosure)
...
/** Map authorized QuantumAI runtimes onto pi Skill records for prompt formatting. */
  ...
  try {
    ...
    if (formatted) {
      return [
        formatted,
        '',
        'QuantumAI policy: call load_skill with the exact skill id (name) before reading files or running scripts. Default-mode skills may already include instructions in this prompt.',
      ].join('\n');
    }
  } catch {
    /* fall through */
