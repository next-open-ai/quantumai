...
function buildSkillMarkdown(skill: AgentSkillRuntime, kebabName: string): string {
  ...
  const body = String(skill.instructions || skill.description || '').trim()
    || `When the user task matches “${skill.name || kebabName}”, follow that skill’s purpose and obey the active QuantumAI workspace-mode contract for the final path.`;
...
/**
 * Materialize QuantumAI-authorized skills into a dsh-discoverable directory:
 * `<cwd>/.agents/skills/<kebab>/SKILL.md`
 *
 * Returns the skills root path when at least one skill was written; otherwise null.
 *
 * Note: QuantumAI progressive disclosure often sends `available` skills without
