...
function buildPrompt(input: ChatRequest, mcpCatalog?: DshMcpCatalog): string {
  ...
  if (profile) {
    lines.push(`[QuantumAI agent profile]\n${profile.slice(0, 8_000)}`);
  }
 ...
 * Injects QuantumAI ModelConfig, MCP connections, and authorized skills into cordis.
 *
 * MCP strategy:
 * 1. Warm/pin connectors in the QuantumAI process pool (first call cold, later hits fast).
    ...
    const env: NodeJS.ProcessEnv = {
      ...
      DSH_SYSTEM_PROMPT: (() => {
        const base = input.profile?.instructions?.slice(0, 4_000)
          || 'You are a careful coding agent working inside a QuantumAI workspace.';
