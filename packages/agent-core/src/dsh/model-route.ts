...
/**
 * Map a QuantumAI ModelConfig onto a single pi-ai hand-declared route.
...
export function mapWorkmateModelToDshRoute(model: ModelConfig): WorkmateDshModelRoute {
  ...
  if (!chatModel) throw new Error('QuantumAI model is missing chatModel.');
  ...
  const displayName = String(model.providerLabel || providerId || 'QuantumAI').slice(0, 80);
  ...
  switch (providerId) {
    ...
    case 'google':
      ...
      // Require an OpenAI-compatible Gemini gateway URL from QuantumAI settings.
      if (!baseUrl) {
        throw new Error(
          'Google/Gemini for dsh requires an OpenAI-compatible baseUrl in QuantumAI model settings '
    ...
    case 'openai-compatible':
      if (!baseUrl) {
        throw new Error(
          `Provider "${providerId}" requires baseUrl in QuantumAI model settings for dsh.`,
        );
      }
      break;
    default:
  ...
  if (!apiKey && providerId !== 'ollama') {
    throw new Error(`QuantumAI model "${chatModel}" is missing apiKey (required for dsh).`);
  }
