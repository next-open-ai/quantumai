...
export function createSkillExecutionTools(input: {
  ...
  const tools: AgentTool[] = [
    ...
    defineAgentTool({
      ...
      description: 'Export the currently published HTML of an existing QuantumAI data app into this run workspace before conversational optimization.',
      parameters: Type.Object({
      ...
      execute: async ({ appId, token, path: relative }) => {
        ...
        const origin = (process.env.WORKMATE_API_ORIGIN?.trim() || `http://127.0.0.1:${process.env.WORKMATE_API_PORT?.trim() || process.env.PORT?.trim() || '4428'}`).replace(/\/$/, '');
        try {
    ...
    defineAgentTool({
      ...
      description:
        'Bind a finished HTML file from this run workspace as the published custom site for a QuantumAI local data app. '
      ...
      execute: async ({ appId, token, path: relative, note }) => {
        ...
        const origin = (
          ...
          || `http://127.0.0.1:${process.env.WORKMATE_API_PORT?.trim() || process.env.PORT?.trim() || '4428'}`
        ).replace(/\/$/, '');
