...
def _system_prompt(params: dict[str, Any]) -> str:
    ...
    parts = [
        f"You are {name} inside QuantumAI (AgentScope engine).",
        instructions,
...
async def run_agentscope_react(
    *,
    ...
    """Drive AgentScope Agent.reply_stream and map events → QuantumAI AgentEvent."""
    ...
    agent = Agent(
        name=str((params.get("profile") or {}).get("name") or "QuantumAI"),
