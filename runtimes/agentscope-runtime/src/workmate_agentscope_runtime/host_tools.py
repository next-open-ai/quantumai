...
class WorkspaceNativeTool(ToolBase):
    ...
    async def check_permissions(self, tool_input: dict[str, Any], context: PermissionContext) -> PermissionDecision:
        ...
        return PermissionDecision(behavior=PermissionBehavior.ALLOW, message=f"QuantumAI workspace policy allows `{self.name}`.")
...
def build_host_toolkit(
    ...
    tools = [
        HostExternalTool(
            name="host_ping",
            description="Health/echo tool executed by the QuantumAI host. Use to verify host tool bridge.",
            input_schema={
                "type": "object",
