# QuantumAI × AgentScope Engine ABI v1
...
QuantumAI UI / orchestrator consume **existing** `@workmate/contracts` `AgentEvent` types. The runtime must emit these shapes inside `agent.event`.
