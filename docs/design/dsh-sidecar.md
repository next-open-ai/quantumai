...
> 状态：ExecutionBackend `dsh` 已注册；按 **QuantumAI ModelConfig** 生成 `llm-pi-ai` cordis。  
...
1. 读取本回合 `ChatRequest.model`（即 QuantumAI 已配置并选中的连接）
...
| QuantumAI `provider` | dsh 协议 | baseUrl |
|---------------------|----------|---------|
...
- QuantumAI 安装包**不**内置 harness；仅薄桥（KB 级）。
...
- QuantumAI `http` / `sse` → dsh `streamable-http`（SSE 端点为 best-effort）
