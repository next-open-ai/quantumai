...
> 本文档是 QuantumAI 的官方用户手册，可在应用内「左侧边栏 → 用户手册」随时打开查看。
...
QuantumAI 是一个**本地优先**的「数字员工」AI 工作台。你可以把不同的 **AI 数字员工** 当作团队里的同事来使用：与它们对话、把任务交给它们执行，让它们调用 **技能（Skills）**、检索 **知识库**、访问 **MCP 连接**、使用 **联网搜索**，并管理运行产生的 **资产** 与 **自动化** 任务。
...
> 模型配置是使用 QuantumAI 的**前置条件**。缺少服务商或模型时，启动会弹出引导提示；你也可以随时在「设置 → Provider / 模型」中修改。
...
「对话工作台」是 QuantumAI 的主界面，用于与数字员工对话、安排任务。
...
- **定时任务**：明确的重复工作可按计划交给数字员工执行（见 [定时任务](#automations)）。
...
数据工作台仍在开发中，当前版本暂不显示菜单入口。其数据导入、模板任务和数据应用能力不参与定时任务流程。

---

## 十一、定时任务 {#automations}

定时任务用于让数字员工在应用运行期间按计划执行一条明确指令。

### 11.1 创建任务

1. 打开左侧「定时任务」。
2. 点击「新建任务」，填写任务名称和执行内容。
3. 选择数字员工、模型以及单次或周期计划。
4. 保存后立即启用，也可以先取消勾选启用状态。

### 11.2 调度与运行

- 可在列表中启用、暂停、编辑或删除任务。
- 可点击「立即执行」进行一次手动验证。
- 列表显示下次执行时间、上次执行时间和最近错误。

### 11.3 注意事项

- 定时任务执行需要模型可用；如果模型不可用，任务会失败并在界面提示。
...
QuantumAI 启动时会自动体检本地环境，缺少组件会弹出提示，你可以在「环境检查」页面查看每一项的「要求 vs 当前」并一键修复。
