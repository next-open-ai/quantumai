...
本轮改造的目标，是让 QuantumAI 同时支持两种发布和运行方式：
...
本 PR 完成 QuantumAI 从单一 Electron 桌面形态向“双运行形态”的关键升级：在保留桌面最优体验的同时，引入 npm Web Launcher，并将 providers、skills、workspace、assets 等主要服务能力从 Electron IPC 下沉到统一 HTTP API。前端页面保持尽量复用，并根据运行环境自动切换系统打开、Finder、下载、inline 预览、新标签页等最佳交互路径。与此同时，AgentScope 迁移主线继续推进，MCP Toolkit 集成、runtime bundling 与 tracing 也已落地。
