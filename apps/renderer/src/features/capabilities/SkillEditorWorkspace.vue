...
<template>
  <section class="flex h-full min-h-0 flex-col bg-[var(--background)]"><header class="flex shrink-0 items-center justify-between border-b border-[var(--border)] px-6 py-4"><div><p class="text-[11px] font-extrabold tracking-[.12em] text-[var(--accent)]">QuantumAI / SKILL WORKSPACE</p><h1 class="mt-1 text-lg font-bold">{{ skill.name }} <span class="font-normal text-[var(--muted)]">/ {{ selectedFile }}</span></h1></div><div class="flex items-center gap-3"><span v-if="isDraft" class="rounded-full bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-700">未保存草案</span><button class="rounded-lg border border-[var(--border)] px-3 py-2 text-sm font-semibold" @click="emit('close')">关闭</button><button class="rounded-lg bg-[var(--accent)] px-3 py-2 text-sm font-semibold text-white disabled:opacity-40" :disabled="saving" @click="persist()">{{ saving ? '正在保存…' : '保存 Skill' }}</button></div></header>
