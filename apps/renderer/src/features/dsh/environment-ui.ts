  ...
  return {
    ...
    script: kind.os === 'windows'
      ...
      : joinScript([
          `mkdir -p "${rootHint}"`,
          `cd "${rootHint}"`,
          '# 推荐在 QuantumAI「环境检查」点一键安装（会装齐 cordis 插件）',
          `npm install @deepseek-ai/dsh-sdk-jsonrpc-demo@${npmVersion} --omit=dev`,
        ]),
    logic: [
      'QuantumAI 通过 stdio JSON-RPC 调用 dsh-jsonrpc-agent，不走 dsh Web。',
      '一键修复会按 QuantumAI cordis 所需插件清单安装到用户数据目录，并优先于 sibling harness。',
    ],
