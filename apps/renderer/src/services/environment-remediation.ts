  ...
  switch (check.id) {
    case 'node':
      ...
      if (kind.os === 'macos') {
        return {
          ...
          steps: ['安装 Node 22 后重启 QuantumAI。', '确认 `node -v` 与 `npx --version` 都可用。'],
    ...
    case 'python':
      if (kind.os === 'macos') {
        return {
          ...
          steps: ['可点「尝试安装 Python 3.12」由本机包管理器安装。', '安装完成后执行 `python3 --version` 验证并重启 QuantumAI。'],
      ...
      if (kind.os === 'windows') {
        return {
          ...
          steps: ['可点「尝试安装 Python 3.12」使用 winget。', '安装后重新打开终端，再重启 QuantumAI。'],
    ...
    case 'electron':
      return {
        ...
        steps: ['请重新安装桌面版 QuantumAI。', 'Web/Docker 形态不依赖 Electron。'],
      };
...
export function buildRemediationPack(report: EnvCheckReport | null): EnvRemediationPack | null {
  ...
  const hostLines = [
    '#!/usr/bin/env bash',
    'set -euo pipefail',
    `# QuantumAI environment remediation pack (${report.platform})`,
    '',
  ];
  ...
  const dockerLines = [
    '# QuantumAI Dockerfile remediation fragments',
