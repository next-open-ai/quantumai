 ...
 * never runs and Vite proxies to a dead :4428. Clear the flag before ready.
 */
...
const apiPort = Number(process.env.WORKMATE_API_PORT || 4428);
...
// A second packaged launch used to start another API on 4428, fail with
 ...
 * macOS, leaving Vite proxying to a dead 4428. Packaged builds still spawn via
...
function startApi() {
  ...
  // Dev supervisor may already own :4428 — do not double-bind.
...
async function waitForApi(timeoutMs = 30_000) {
  ...
  throw new Error(`QuantumAI local API did not become ready within ${timeoutMs}ms.`);
}
...
async function createWindow() {
  ...
  mainWindow.webContents.on('did-fail-load', (_event, code, description, validatedUrl) => {
    ...
    if (!String(validatedUrl || '').includes('127.0.0.1:5273')) return;
    rendererRetry += 1;
    setTimeout(() => {
      if (!mainWindow || mainWindow.isDestroyed()) return;
