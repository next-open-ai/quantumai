...
async function waitForApi(timeoutMs = 30_000) {
  ...
  throw new Error(`QuantumAI API did not become ready on :${apiPort}`);
}
...
function startApiProcess() {
  ...
  return (async () => {
    ...
    if (!replacingOwnedApi) {
      ...
      if (existingApi) {
        if (!await hasLanApi()) {
          throw new Error(`An existing API on :${apiPort} is loopback-only. Stop the legacy QuantumAI dev process before restarting.`);
        }
          api = undefined;
...
async function startUi() {
  ...
  const rendererUrl = 'http://127.0.0.1:5273/';
