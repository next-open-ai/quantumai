...
function apiListenPort() {
  return Number(process.env.WORKMATE_API_PORT || 4428);
}
...
export const dataRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/data/apps', async (request) => {
    ...
    const host = String(request.headers.host || '127.0.0.1:4428');
    return rows.map((row) => {
  ...
  app.post('/data/import', { bodyLimit: 12 * 1024 * 1024 }, async (request, reply) => {
    ...
    try {
      ...
      // Uploaded databases are inspected in memory and copied into QuantumAI's
...
export const publicDataAppRoutes: FastifyPluginAsync = async (app) => {
  app.get('/data-mobile-upload/:token', async (request, reply) => {
    ...
    if (!row || Number(row[0]) < Date.now()) return reply.code(410).type('text/html; charset=utf-8').send('<!doctype html><meta charset="utf-8"><title>上传链接已过期</title><body style="font:16px sans-serif;padding:40px">上传链接已过期，请回到 QuantumAI 重新生成二维码。</body>');
